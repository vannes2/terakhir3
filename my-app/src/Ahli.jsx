...
const Ahli = () => {
  ...
  // Fungsi untuk navigasi ke halaman RincianBayar (dimulai dari nama dokter)
  const handleNavigateToRincianBayar = (dokter) => {
    navigate("/rincianbayar", { state: { dokter } }); // Kirim data dokter sebagai state
  };

  // Fungsi untuk navigasi ke halaman Consul (opsional, bisa tetap digunakan untuk gambar)
        ...
        <div className="dokter-grid">
          {filteredDokters.map((dokter) => (
            <div
              ...
              onClick={() => handleNavigateToRincianBayar(dokter)} // Navigasi ke rincianbayar.jsx
              style={{ cursor: "pointer" }} // Gaya pointer untuk menunjukkan bahwa ini bisa diklik
            >
              <div className="dokter-left">
                {/* Foto Dokter tetap mengarah ke halaman Consul */}
                <img
                  className="dokter-image"
                  src={`http://localhost/assets/images/${dokter.gambar}`}
                  alt={dokter.nama_dokter}
                  onClick={() => handleNavigateToConsul(dokter)} // Navigasi ke consul.jsx
                  style={{ cursor: "pointer" }}
                />
              </div>
              <div className="dokter-right">
                {/* Nama Dokter mengarah ke RincianBayar */}
                <h2
                ...
                <p className="dokter-experience">
                  ...
                  <span className="rating">⭐</span> {dokter.rating}
                </p>
                ...
                >
                  ...
                  >
                    {dokter.is_available === 1 ? "Tersedia" : "Tidak Tersedia"}
                  </span>
                </div>
              </div>
            </div>
