...
const ProdukList = () => {
    ...
    const filteredProduk = produk.filter((item) => {
        const filterTipeKulit =
            selectedTipeKulit === "all" || item.id_tipe_kulit === parseInt(selectedTipeKulit);
        const filterMasalahKulit =
            selectedMasalahKulit === "all" || item.id_masalah === parseInt(selectedMasalahKulit);
    ...
    return (
        <div style={{ backgroundColor: '#FFFFFF', padding: '20px' }}>
            ...
            <section className="flex justify-center gap-10 px-[120px] py-[20px]">
                <div>
                    <label htmlFor="filter-tipe-kulit" className="font-bold">Tipe Kulit:</label>
                    <select
                ...
                <div>
                    <label htmlFor="filter-masalah-kulit" className="font-bold">Masalah Kulit:</label>
                    <select
            ...
            {Object.keys(groupedProduk).length === 0 ? (
                ...
                Object.keys(groupedProduk).map((jenisNama) => (
                    groupedProduk[jenisNama].length > 0 && (
                        <section key={jenisNama}>
                            ...
                            >
                                <h2 style={{ fontSize: '20px', fontWeight: 'bold' }}>{jenisNama}</h2>
                                <Link
                                    to="#"
                            ...
                            <div style={{
                                ...
                                {groupedProduk[jenisNama].map((item) => (
                                    ...
                                    >
                                        ...
                                        <h4 style={{ color: '#147A63', fontWeight: 'bold' }}>{item.nama_brand}</h4>
                                        <h3>{item.nama_produk || 'Produk Tanpa Nama'}</h3>
                                        <p><strong>Harga:</strong> {item.kisaran_harga}</p>
                                    </div>
                                ))}
                            </div>
                        </section>
